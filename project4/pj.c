...
void random_reference_string(int page_string[]); // 1~30을 500번 돌린 후 page_string 배열에 넣어주는 함수 
void cleanBuffer(void); // 입력 버퍼 비워주는 함수
int menu_b(int, char fin[4]); // B메뉴 호출 함수
int menu_c(int, char fin[4]); // C메뉴 호출 함수
void do_menu_c(FILE *f1, FILE *f2, int input_data, int page_string[500]); // C 메뉴 입력 시 값에 따라 작업을 수행하는 함수.
int check_index(int page_frame, int *pageArray, int n); // pageArray에 해당 참조 스트림 값이 있는지 확인하는 함수
int nearIndex(int pf, int cur, int *pageArray, int n); // 가장 가까이 있는 값 받아오는 함수
int checkMax(int pf, int *pageArray, int check_value); // 가장 멀리 있는 값 받아오는 함수
void opt(int page_string[500], int page_frame, int *pageArray); // optimal 
...
void initQueue(Queue *queue); // LRU에서 큐를 초기화하는 함수
int isEmpty(Queue *queue); // LRU에서 링크드리스트에 값이 비어있는 지 확인하는 함수
int getQueue(Queue *queue, int data); // LRU에 새로운 값 삽입 함수
void deQueue(Queue *queue); // LRU에 값 삭제 함수
int searchQueue(Queue *queue, int value); // LRU - Queue에 해당 값이 있는지  탐색하는 함수.
void hitQueue(Queue *queue, int value); // LRU - hit발생 시 수행 함수.
...
int main()
{
	while (1)
	{
		...
		char resultStr[50] = {0}; 	// 선택하신 메뉴 띄워주는 문장.
		char ch;					// int를 char로 변환해서 쓰기 위한 변수
		int len;					// 배열 길이 가져오는 함수.
		...
		FILE *f1, *f2, *fp;	// 파일 입력 및 출력을 위한 파일포인터
		...
		// 결과 파일 저장해야 함.
		printf("결과 파일이 저장되었습니다.(결과 파일 : 20182625.txt)\n\n\n");
	} // A메뉴에 대한 무한루프 종료.
	return 0;
}
...
void do_menu_c(FILE *f1, FILE *f2, int input_data, int page_string[500])
{
	...
	else if (input_data == 2)
	{
		...
		if (f1 == NULL)
			...
			printf("파일을 정상적으로 만들었습니다!(파일 이름 : reference_string.txt\n");
