...
void lru(int page_string[500], int page_frame, int *pageArray);
	...
	while(1)
	{
		...
		if (flagEight ==0) // 8번 메뉴를 선택하지 않았을 때
		{
			...
			for (int i=0; i<10; i++)
			{
				...
				{
					//4번 LRU 선택 시
					lru(page_string, page_frame, pageArray);
				}
...
void lru(int page_string[500], int page_frame, int *pageArray)
{
	...
	int t;
	int block = 0;
	int pIndex[page_frame]; // 이전 인덱스를 의미하는 배열
	...
	for (t=0; t<500; t++)
	{
		...
		else
		{
			if(block == page_frame) 
			{
				...
				for (; tmp < page_frame; tmp++)
				{
					pIndex[tmp] = nearIndex(500, t, page_string, pageArray[tmp], -1);
				}
				int goal = checkMax(page_frame, pIndex, -1);
				pageArray[goal] = page_string[t];
			}
